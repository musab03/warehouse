...
function FAQ() {
  ...
  <div className="hs-accordion-group divide-y divide-black">
    ...
    {[
      ...
      {
        question: "How can I track my inventory and manage my stored goods?",
        answer:
          "Tulip Bond offers a real-time inventory management system that provides updates on stock levels, storage duration, and customs clearance status via an online portal."
      }
    ].map((item, index) => (
      <div key={index} className="hs-accordion pt-6 pb-3">
        ...
        >
          {item.question}
          <svg
            className={`hs-accordion-active:hidden block shrink-0 size-5 text-gray-600 group-hover:text-gray-500 ${open === index ? "rotate-180" : ""}`}
          ...
          <svg
            className={`hs-accordion-active:block hidden shrink-0 size-5 text-gray-600 group-hover:text-gray-500 ${open === index ? "rotate-180" : ""}`}
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
        ...
        <div
          className={`hs-accordion-content w-full overflow-hidden transition-[height] duration-300 ${open === index ? "block" : "hidden"}`}
          role="region"
